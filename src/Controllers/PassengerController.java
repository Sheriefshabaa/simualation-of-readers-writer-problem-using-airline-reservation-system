...
public class PassengerController extends Passenger implements Runnable {
    ...
    public void createPassenger () {
        ...
        System.out.println("Passenger name\t\tTicket ID\tPhone\tFlight status\tSeat Number\tFlight ID\tAir line\t\tdeparture\t\t\t Landing\t\t\t  Duration");
        while (totalSeats > 0) {
                ...
                if (bookedSeats>totalSeats){
                    for (int i = 1; i <= bookedSeats; i++) {
                        ...
                        System.out.print(pass.getFirstName() + " " + pass.getLastName() + "  \t\t" + ticketController.getTicketId()+
                                ...
                                delayedFlight.getTo_loc() + "\t\t" + delayedFlight.getDuration() + "\n");}
                }
                else {
                    ...
                    for (int i = 1; i <= bookedSeats; i++) {
                        ...
                        System.out.print(pass.getFirstName() + " " + pass.getLastName() + "  \t\t" + ticketController.getTicketId()+
                                ...
                                currentFlight.getTo_loc() + "\t\t" + currentFlight.getDuration() + "\n");
